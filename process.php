...
function doSubmitApplication()
{
	try {
		...
		if (!empty($response->error) or !isset($response)) {
			...
			throw new Exception(isset($response->error) ? $response->error : "No existes asi como para ella", 1);
		}
...
function doInsert($jobid = 0, $fileid = 0)
{
	if (isset($_POST['submit'])) {
		...
		message("Tu solicitud ya fue enviada. Espere la confirmación de la empresa si está calificado para este trabajo.
			.", "success");
...
{
	$target_dir = web_root."admin/applicants/cv/";
